...
func _ready() -> void:
	...
	# belong to each group. This method is not so clean, but accessing this information
	# directly from the element classes would required them each to register in the
	# Godot class database... this is much less annoying
...
func _on_mouse_pressed(start: Vector2, end: Vector2) -> void:
	...
	if start.distance_to(end) > Settings.brush_size / 2.0:
		if abs(end.y - start.y) / abs(end.x - start.x) < 1:
			var wy: float = Settings.brush_size * sqrt(pow(end.x - start.x, 2) + pow(end.y - start.y, 2)) / (2 * abs(end.x - start.x))
			...
			var wx: float = Settings.brush_size * sqrt(pow(end.x - start.x, 2) + pow(end.y - start.y, 2)) / (2 * abs(end.y - start.y))
			var i: int = 0
...
func draw_pixel(row: float, col: float) -> void:
	...
	# Some elements are internally represented by more than one ID, so we shuffle them here
