...
public:
    ...
    void process(SandSimulation *sim, int row, int col) override {
        if (sim->touch_count(row, col, 164) > 3) {
            ...
            if (sim->randf() < FIRE) {
                sim->grow(row - 2, col, 0, 165);
            }
            int dir = (int) (sim->randf() * 2);
            switch (dir) {
                case 0: sim->grow(row + 1, col, 0, 164); sim->grow(row - 1, col, 0, 164); break;
